...
def app():
    ...
    blur = st.sidebar.selectbox("Blur Faces (Privacy)", options=["Yes", "No"])
    st.sidebar.write("---")
    ...
    if mode == "Image":
        ...
        if uploaded_file is not None:
            with st.spinner("Processing image progress"):
                process_uploaded_file(model_weights, uploaded_file, conf_thresh, iou_thresh, device, output_path, blur)
    ...
    if mode == "Video":
        ...
        if video_file is not None:
            if st.button("Process Video"):  
                ...
                process_uploaded_file(model_weights, video_file, conf_thresh, iou_thresh, device, output_path, blur)
